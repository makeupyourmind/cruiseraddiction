...
//dd($partsList);
    // }
        ...
        for($i = 0; $i < count($partNums); $i++) {
            $txt = str_replace("-", '', str_replace('"', '', str_replace("'", '',$partNums[$i])));
            $part = Part::where('part_number', $txt)->first();
            $object = new \stdClass();
            $object->brand_name = $part->brand_name;
            $object->part_number = $part->part_number;
            $object->description_english = $part->description_english;
            $object->weight_physical = $part->weight_physical;
            $object->images = $part->images;
            $object->data = [];
            $data = new \stdClass();
            $data->warehouses = $part->warehouse;
            $data->available = $part->qty;
            $data->prices = $part->price;
            $data->unique_hashes = $part->unique_hash;
